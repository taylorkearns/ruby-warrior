...
describe Attacker do
  ...
  describe '#relevant?' do
    it 'is true if player has a clear shot at an enemy' do
      @attacker.stub(:captive_at?).with(0) { false }
      @attacker.stub(:enemy_at?).with(0) { false }

      @attacker.stub(:captive_at?).with(1) { false }
      @attacker.stub(:enemy_at?).with(1) { false }

      @attacker.stub(:captive_at?).with(2) { false }
      @attacker.stub(:enemy_at?).with(2) { true }
    ...
    it 'is false if player does not have a clear shot at an enemy' do
      ...
      @attacker.stub(:captive_at?).with(1) { true }
      @attacker.stub(:enemy_at?).with(1) { false }
    ...
    it 'is false if no enemy is in range' do
      (0..2).each do |i|
        @attacker.stub(:captive_at?).with(i) { false }
        @attacker.stub(:enemy_at?).with(i) { false }
      end
